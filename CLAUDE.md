...
| Skill                | Description                                                             |
| -------------------- | ----------------------------------------------------------------------- |
| `/assemble-docs`     | Orchestrate all agents to assemble a complete README.md                 |
| `/create-feature`    | Scaffold and build a new frontend feature following project conventions  |
| `/create-issue`      | File any bug or finding as a structured GitHub issue via `gh issue create` |
| `/diagram-arch`      | Generate Excalidraw architecture diagrams → `docs/diagrams/`            |
| `/evaluate-library`  | Evaluate adding or replacing a lib/tool — Go / Conditional Go / Not Needed / No-Go |
| `/migrate-drizzle`   | Migrate from Prisma 7 to Drizzle ORM — schema, db singleton, auth adapter, queries |
| `/fix-bug`           | Surgically fix a specific bug — reads only affected files, min change   |
| `/refactor-code`     | Structural improvements to existing code — no behaviour changes         |
| `/resolve-issue`     | Pick an open GitHub issue, fix it, commit with `Fixes #N`, then push or open PR |
| `/review-design`     | UI/UX review of all components in `src/components/`                     |
| `/review-frontend`   | Full frontend review — lint, typecheck, best practices                  |
| `/setup-ci`          | Bootstrap the full quality pipeline — GitHub Actions, Lefthook, branch protection, CodeRabbit |
