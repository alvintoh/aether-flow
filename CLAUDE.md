...
| `/gh-issue`       | File any bug or finding as a structured GitHub issue via `gh issue create`       |
| `/gh-resolve`     | Pick an open GitHub issue, fix it, commit with `Fixes #N`, then push or open PR |
| `/design-review`  | UI/UX review of all components in `src/components/`                     |
| `/dx-codespace`   | Audit and fix Codespaces setup — devcontainer, ports, auth URL, secrets |
